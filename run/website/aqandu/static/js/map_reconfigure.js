...
function setupMap() {
    ...
    imageBounds = [
        [40.598850, -112.001349],
        [40.810476, -111.713403]
    ];
...
function drawSensorOnMap() {
    ...
    getDataFromDB(liveSensorURL_all).then((data) => {
        ...
        data.forEach((d) => d.PM2_5 = convertPM(d.PM2_5, d['Sensor source'], d['Sensor model']));
        ...
        data.forEach(function(aSensor) {
            liveSensors.push({ 'id': aSensor['Sensor ID'].split(' ').join('_'), 'sensorSource': aSensor['Sensor source'] });
        });
...
function createMarker(markerData) {
    ...
    if (markerData.Latitude !== null && markerData.Longitude !== null) {
        ...
        mark.id = markerData['Sensor ID'];

        mark.bindPopup(
            L
            .popup({ closeButton: false, className: 'sensorInformationPopup' })
            .setContent(`<span class="popup">${sensorSource}: ${markerData['Sensor ID']}</span>`)
        )
...
function getGraphData(sensorID, sensorSource, aggregation) {
    ...
    let parameters = aggregation ? { 'id': sensorID, 'sensorSource': sensorSource, 'start': formatDateTime(pastDate), 'end': formatDateTime(todaysDate), 'function': 'mean', 'timeInterval': '5' } : { 'id': sensorID, 'sensorSource': sensorSource, 'start': formatDateTime(pastDate), 'end': formatDateTime(todaysDate) };

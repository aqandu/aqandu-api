...
class PARAMS_HELP_MESSAGES(str, Enum):
    ...
    AREA_MODEL_AS_LIST = f"Single or multiple (comma separated), specifies a particular area model to use for this query. Area parameters include: {', '.join(list(common.jsonutils.get_all_region_info().keys()))}"
    ...
    DEVICE = "The 12-digit device name, in all caps"
    NICKNAME = "The nickname. The name must be URL-encoded before being inserted into the query. Use a site like urlencoder.org to encode it. It can up to 128 digits alphanumeric or include spaces, ~, or - "
...
def multi_area(value):
    allAreas = list(common.jsonutils.get_all_region_info().keys())
    value = value.split(',')
    if value == ['all']:
        return allAreas
...
def nickname_check(value):
    if bool(re.match(r'^[ -~]{1,128}$', value)):
        return value
    else:
        raise ValueError(PARAMS_HELP_MESSAGES.NICKNAME)